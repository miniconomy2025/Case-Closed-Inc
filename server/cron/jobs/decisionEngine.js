import {
  ...
  getAvailableMaterialStockCount,
...
export default class DecisionEngine {
  ...
  async buyMaterial(state, material) {
    ...
    return (
      ...
      balance > this.thresholds.excessCashThreshold
    );
  }
  ...
  async run() {
    ...
    if (have_account) {
      ...
      if (await this.buyMaterial(state, "plastic")) {
        try {
          logger.info(
            "[DecisionEngine]: Plastic stock low!  Buying 1000 units"
          );
      ...
      if (await this.buyMaterial(state, "aluminium")) {
        try {
          logger.info(
            "[DecisionEngine]: Aluminium stock low! Buying 1000 units"
          );
          await OrderRawMaterialsClient.processOrderFlow({
            name: "aluminium",
            quantity: 1000,
      ...
      try {
        const { accountNumber } = await BankClient.createAccount({
          notification_url:
            "https://case-supplier-api.projects.bbdgrad.com/api/payment",
        });
        ...
        logger.error(`[DecisionEngine]: Failed to create account: ${error}`);
